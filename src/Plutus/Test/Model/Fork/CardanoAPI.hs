...
castHash ::
  forall a b.
  (C.SerialiseAsRawBytes (C.Hash a), C.SerialiseAsRawBytes (C.Hash b)) =>
  String ->
  C.Hash a ->
  Either ToCardanoError (C.Hash b)
castHash msg =
...
toCardanoPlutusScript =
  tag "toCardanoPlutusScript"
    . deserialiseFromRawBytes (C.AsPlutusScript C.AsPlutusScriptV1)
    . BSL.toStrict
    . Codec.serialise
