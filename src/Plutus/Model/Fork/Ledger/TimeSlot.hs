...
module Plutus.Model.Fork.Ledger.TimeSlot(
  SlotConfig(..)
, SlotConversionError(..)
...
import Plutus.Model.Fork.Ledger.Slot (Slot (Slot), SlotRange)
import PlutusLedgerApi.V1.Interval (Extended (..), Interval (Interval), LowerBound (..), UpperBound (..), interval,
                                  member)
