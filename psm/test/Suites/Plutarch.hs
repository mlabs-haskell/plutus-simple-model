...
testValidator guess = do
  ...
  let amt = adaValue 100
      datum = toBuiltin $ plift $ phexByteStr "961b6dd3ede3cb8ecbaacbd68de040cd78eb2ed5889130cceb4c49268ea4d506"
  -- from 'echo -n aa | sha256sum'
  do
    -- initialize
    sp <- spend u1 amt
    tx <-
...
validator =
  fromRight (error "no validator") $
    mkTypedValidatorPlutarch def {tracingMode = DoTracing} gameContractPlutarch
