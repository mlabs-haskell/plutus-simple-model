module Suites.Plutus.Model.Script.V2.Test.Oracle (
  tests
) where
...
import Suites.Plutus.Model.Script.V2.Onchain.Oracle.Script
...
tests cfg =
  testGroup
    "Oracle Bet scripts (reference inputs)"
...
postAnswer oraclePkh answer = do
  ...
  submitTx oraclePkh $
    mconcat
      [ userSpend usp
      , payWithDatumToPubKey oraclePkh (InlineDatum (AnswerDatum answer)) riderAda
      ]
...
victoryBy Fraud{..} app@App{..} user oracle = do
  mOracleRef <- fmap fst . L.find (hasOracleDatum . snd) <$> utxoAt oracle
  case mOracleRef of
    Nothing        -> logError "No oracle answer"
    Just oracleRef -> do
      betBox <- getBet app
  ...
  where
    victoryTx oracleRef betBox =
      mconcat
        ...
        , fraud'refInput $ refInput oracleRef
